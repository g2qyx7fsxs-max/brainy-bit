import { RelatedBook, RelatedEvent, RelatedFigure } from "@/lib/content";
...
export function RelatedRecommendations({
  ...
  if (!books?.length && !events?.length && !figures?.length) return null;
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      {figures?.length ? (
        <div className="rounded-2xl bg-gray-50 p-5">
          <p className="mb-3 text-sm font-bold text-gray-900">🧑‍🔬 Meet the people behind this</p>
          <ul className="space-y-2 text-sm text-gray-700">
            {figures.map((figure) => (
              <li key={figure.name}>
                <span className="font-medium">{figure.name}</span>
                <span className="text-gray-500"> — {figure.description}</span>
              </li>
            ))}
          </ul>
        </div>
